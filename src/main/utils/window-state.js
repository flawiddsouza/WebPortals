...
const electron = require('electron') // eslint-disable-line @typescript-eslint/no-require-imports
const jsonfile = require('jsonfile') // eslint-disable-line @typescript-eslint/no-require-imports
const mkdirp = require('mkdirp') // eslint-disable-line @typescript-eslint/no-require-imports
...
export default function (options) {
  ...
  function manage(win) {
    if (config.fullScreen && state.isFullScreen) {
      ...
      // win.maximize(); // moved to show handler as it was causing issues
    }
  ...
  function calcScale() {
    ...
    if (display) {
      console.log(
        `"right top point: ${screen.getPrimaryDisplay().scaleFactor} / ${display.scaleFactor}"`
      )
      return screen.getPrimaryDisplay().scaleFactor / display.scaleFactor
    }
    return 1
